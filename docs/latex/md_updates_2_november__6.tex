...
In the following sections I\textquotesingle{}ll talk about some of the things that I\textquotesingle{}ve implemented.\hypertarget{md_updates_2_november__6_autotoc_md1}{}\doxysubsection{\texorpdfstring{Application}{Application}}\label{md_updates_2_november__6_autotoc_md1}
...
It calls currently calls to update functions. The window {\bfseries{on\+Update}} function just calls {\bfseries{glfw\+Poll\+Events}} and the application {\bfseries{on\+Update}} isn\textquotesingle{}t implemented yet.
...
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}KeyboardEvent:\ keycode=\{\},\ action=\{\}"{}},\ data.keycode,\ data.action);}
...
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}MouseMoveEvent:\ x=\{\},\ y=\{\}"{}},\ data.xPos,\ data.yPos);}
\DoxyCodeLine{\ \ \ \ \ \ \textcolor{keywordflow}{break};}
\DoxyCodeLine{\ \ \ \ \}}
...
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}MouseButtonEvent:\ button=\{\},\ action=\{\}"{}},\ data.button,\ data.action);}
\DoxyCodeLine{\ \ \ \ \ \ \textcolor{keywordflow}{break};}
\DoxyCodeLine{\ \ \ \ \}}
...
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}MouseScrollEvent:\ x=\{\},\ y=\{\}"{}},\ data.xScroll,\ data.yScroll);}
\DoxyCodeLine{\ \ \ \ \ \ \textcolor{keywordflow}{break};}
\DoxyCodeLine{\ \ \ \ \}}
...
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}ResizeEvent:\ width=\{\},\ height=\{\}"{}},\ data.newWidth,\ data.newHeight);}
\DoxyCodeLine{\ \ \ \ \ \ \textcolor{keywordflow}{break};}
\DoxyCodeLine{\ \ \ \ \}}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordflow}{case}\ Event::Type::Close:\ \{}
\DoxyCodeLine{\ \ \ \ \ \ LOGI(\textcolor{stringliteral}{"{}CloseEvent"{}});}
\DoxyCodeLine{\ \ \ \ \ \ \textcolor{keywordflow}{break};}
\DoxyCodeLine{\ \ \ \ \}}
\DoxyCodeLine{\ \ \}}
\DoxyCodeLine{\}}
...
This simple system will probably work fine for everything we need. In the future we might need to forward the events to the gui, but that can just be a method call from a gui class.\hypertarget{md_updates_2_november__6_autotoc_md2}{}\doxysubsection{\texorpdfstring{Window}{Window}}\label{md_updates_2_november__6_autotoc_md2}
...
These settings are then used to initialize glfw. Because we\textquotesingle{}re only going to use vulkan, there is no need to abstract the window class, as we\textquotesingle{}ll only ever be using glfw.

This is also where the {\bfseries{event\+Callback}} gets called. This is one example.
...
It uses {\bfseries{glfw\+Get\+Window\+User\+Pointer}} to get the window\+Data and then uses the callback which gets set in the application.\hypertarget{md_updates_2_november__6_autotoc_md3}{}\doxysubsection{\texorpdfstring{Events}{Events}}\label{md_updates_2_november__6_autotoc_md3}
...
The type can then be checked in switch statement and the data can be accessed.\hypertarget{md_updates_2_november__6_autotoc_md4}{}\doxysubsection{\texorpdfstring{Input Polling}{Input Polling}}\label{md_updates_2_november__6_autotoc_md4}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\ ++}
\DoxyCodeLine{\textcolor{preprocessor}{\#define\ LOGI(...)\ spdlog::info(\_\_VA\_ARGS\_\_);}}
\DoxyCodeLine{\textcolor{preprocessor}{\#define\ LOGW(...)\ spdlog::warn(\_\_VA\_ARGS\_\_);}}
\DoxyCodeLine{\textcolor{preprocessor}{\#define\ LOGE(...)\ spdlog::error("{}[\{\}:\{\}]\ \{\}"{},\ \_\_FILENAME\_\_,\ \_\_LINE\_\_,\ fmt::format(\_\_VA\_ARGS\_\_));}}
\DoxyCodeLine{\textcolor{preprocessor}{\#define\ LOGD(...)\ spdlog::debug(\_\_VA\_ARGS\_\_);}}
\DoxyCodeLine{}
\DoxyCodeLine{\textcolor{keyword}{namespace\ }\mbox{\hyperlink{namespacevkf_1_1logging}{vkf::logging}}\ \{}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordtype}{void}\ \mbox{\hyperlink{namespacevkf_1_1logging_a93e6587a47f3fefe0616fde68f6cf2e0}{init}}();}
\DoxyCodeLine{\}\ \textcolor{comment}{//\ namespace\ vkf}}

\end{DoxyCode}
\hypertarget{md_updates_2_november__6_autotoc_md6}{}\doxysubsection{\texorpdfstring{Vulkan Hpp}{Vulkan Hpp}}\label{md_updates_2_november__6_autotoc_md6}
This project uses \href{https://github.com/KhronosGroup/Vulkan-Hpp}{\texttt{ vulkan.\+hpp}}. This means the code will look a bit different to standard Vulkan API. Because this project is compiled using the c++20 standard, I also use designated initializers. In order to use them you need to \texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*}\#define VULKAN\+\_\+\+HPP\+\_\+\+NO\+\_\+\+CONSTRUCTORS\texorpdfstring{$\ast$}{*}\texorpdfstring{$\ast$}{*}. This makes the code look cleaner in my opinion. Here an example from the vulkan hpp git\+Hub.
...
\end{DoxyCode}
\hypertarget{md_updates_2_november__6_autotoc_md7}{}\doxysubsubsection{\texorpdfstring{Dynamic Loader}{Dynamic Loader}}\label{md_updates_2_november__6_autotoc_md7}
...
There is a detailed explanation in the git\+Hub README.\+md\hypertarget{md_updates_2_november__6_autotoc_md8}{}\doxysubsubsection{\texorpdfstring{Raii}{Raii}}\label{md_updates_2_november__6_autotoc_md8}
...
This device gets destroyed automatically when it leaves scope. There are other benefits, but also some inconveniences. All of this can be found in the \href{https://github.com/KhronosGroup/Vulkan-Hpp/blob/main/vk_raii_ProgrammingGuide.mdk}{\texttt{ programing guide}}.\hypertarget{md_updates_2_november__6_autotoc_md9}{}\doxysubsection{\texorpdfstring{Vulkan Instance and Debug Messenger}{Vulkan Instance and Debug Messenger}}\label{md_updates_2_november__6_autotoc_md9}
