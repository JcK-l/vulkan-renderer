...
// texture file.
///
struct GeotiffComponent
{
    struct Data
    {
        glm::vec4 cornersData;
        float poleLat;
        float poleLon;
        float colorIntensity; // 0..1 with 1 = rgb texture used, 0 = grey scales
        float padding;        // padding to make the struct 32 bytes because of base alignment requirements
    };

    GeotiffComponent(const core::Device &device); ///< Constructor
